...
def other_corpus(db):
    ...
    for i in range(20):
        drafts.append(DocumentDraft(
            source="other", source_key=f"phone{i}", doc_type="shopping",
    ...
    for i in range(20):
        drafts.append(DocumentDraft(
            source="other", source_key=f"charger{i}", doc_type="shopping",
    ...
    for i in range(20):
        drafts.append(DocumentDraft(
            source="other", source_key=f"case{i}", doc_type="shopping",
...
def test_iphone_returns_an_actual_phone_on_page_one(other_corpus):
...
@pytest.mark.django_db
def test_no_more_than_three_consecutive_results_share_a_leaf_category(other_corpus):
...
@pytest.mark.django_db
def test_a_query_that_names_an_accessory_still_returns_accessories(other_corpus):
...
@pytest.mark.django_db
def test_category_is_available_as_a_facet(other_corpus):
