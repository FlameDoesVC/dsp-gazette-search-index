"""
Django settings for gazette_search project.
...
ROOT_URLCONF = 'gazette_search.urls'
...
WSGI_APPLICATION = 'gazette_search.wsgi.application'
...
# `or` rather than `.get(..., default)`: an env file that declares the key
# with no value (e.g. a freshly-copied .env.example) sets it to "", which is
# present, not absent, so the dict default alone would never catch it.
_default_url = os.environ.get("DATABASE_URL") or f"sqlite:///{BASE_DIR / 'db.sqlite3'}"
_direct_url = os.environ.get("DATABASE_URL_DIRECT") or _default_url
...
# ollama's own prompt_eval_count rather than guessed: 11 non-gazette job calls
# reported 24,182 input tokens for prompts of about 7,098 characters, so 3.2
# for that mix. 2.8 keeps a margin for heavier Thaana without crying wolf -- at the
...
# a repetition loop and keeps emitting until it fills num_ctx -- a single
# non-gazette JOB document took 282 seconds and others passed the 300s mark, at concurrency
